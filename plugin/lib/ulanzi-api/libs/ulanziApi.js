...
export default class UlanziApi extends EventEmitter {
  ...
  connect(uuid, port = 3906, address = "127.0.0.1") {
    ...
    this.websocket.onmessage = (evt) => {
      ...
      // 原始入站消息打点(排查 getGlobalSettings 回复为何不触发 didReceiveGlobalSettings)
      try {
        if (typeof window !== "undefined" && window.PetLog) {
          window.PetLog.debug("ws recv", {
            cmd: data && data.cmd,
            cmdType: data && data.cmdType,
